...
def second():
    AutoScalingGroupName = request.form['username']
    #username = "abcd"
...
def attach():
    AutoScalingGroupName = request.form['asgname']
    ...
    return (render_template("Instances is Attached"))
...
@app.route('/my-link/')
def my_link():
        return render_template('c.html',tables=[df.to_html(classes='data')] ,titles=df.columns.values)    
...
def my_form_post():
    ...
    p = ASG_tag.index(text)
    abc =  (df.iloc[[p]])
    #print(abc)
    return render_template('c.html',tables=[abc.to_html(classes='data')] ,titles=abc.columns.values)
